...
x.AnonFuncExpression = function(params, body) {
	...
	var f = x.FuncExpression(null, params, x.BlockExpression(body));
	f._anon = true;
	return f;
};
...
x.ForLoop = function(varId, fromExpr, toExpr, stepExpr, body) {
	...
	return {
		type: "ForStatement",
		init: x.VarDeclaration("var", varId, fromExpr, true),
...
x.ForEachLoop = function(iter, varId, body) {
	...
	body.body.unshift(x.VarDeclaration("var", varId, x.MemberExpression(iter, iterId)));
	
	return {
		type: "ForStatement",
		init: x.VarDeclaration("var", iterId, x.NumericLiteral("0"), true),
...
x.VarDeclaration = function(type, id, init, rawId) {
	if(init === undefined) {
		init = null;
	}
